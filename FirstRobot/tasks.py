...
def solve_challenge():
    ...
    Downloads the source data Excel file and uses Playwright to fill the
    entries inside rpachallenge.com.
    """
    browser.configure(
        browser_engine="chromium", screenshot="only-on-failure", headless=False
    )

    # Get the challenge URL
    secrets = vault.get_secret('DemoFirstRobotURL')
    locators = storage.get_json('FirstRobotLocators')
    ...
    try:
        ...
        # Surf the automation challenge website and fill in information from
        # the table extracted above.
        page = browser.goto(secrets['url'])  # Paste the challenge URL
        ...
        # A place for teardown and cleanups. (Playwright handles browser
        # closing)
...
def download_file(url: str, *, target_dir: Path, target_filename: str) -> Path:
    ...
    Args:
        ...
        target_filename: The local file name inside which the content gets
        saved.
    ...
    # this will raise an exception if the requestfails
